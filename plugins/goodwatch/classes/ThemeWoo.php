...
class ThemeWoo {
						...
						if ( $attribute->is_taxonomy() ) {
							...
							foreach ( $attribute_values as $attribute_value ) {
								...
								if ( $attribute_taxonomy->attribute_public ) {
									$link     = '/shop/?' . preg_replace( '/pa/', 'filter', $attribute->get_name() ) . '=' . $attribute->get_slugs()[0];
									$values[] = '<a href="' . esc_url( $link ) . '" rel="tag">' . $value_name . '</a>';
								} else {
									$values[] = $value_name;
								}
							}
	...
	public function metaTags() {
		...
		add_filter( 'woocommerce_page_title', function () {
			...
			if ( empty( $pch ) ) {
				echo '<h1 class="woocommerce-products-header__title page-title main-title mb-4">' . get_queried_object()->name . '</h1>';
			} else {
				echo '<h1 class="woocommerce-products-header__title page-title title main-title mb-4">' . $pch . '</h1>';
			}
		}, 10, 2 );
